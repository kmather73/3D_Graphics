...
function createStairs() {
	...
	for(var i = 0; i < 6; ++i){
        ...
        stepMesh.position.z = i * (stepWidth) - 6*i*stepHalfThickness ;		// centered at origin
        scene.add( stepMesh );
        ...
        stepMesh.position.y = stepThickness/2 + (i+1) * verticalStepHeight + i*stepThickness;
        // Push step forward by half the depth, minus half the vertical step's thickness
        stepMesh.position.z = (i + 1/2)*horizontalStepDepth - (1+2*i)*stepHalfThickness;
        scene.add( stepMesh );
    }
}
